...
<section class="section"><div class="head"><div class="num">02</div><div><h2>{{t "phone.receiveTitle"}}</h2><p>{{t "phone.receiveDescription"}}</p></div></div><div class="actions"><button id="refreshButton" class="button" type="button">{{t "phone.refresh"}}</button></div><div id="receiveHint" class="receiveHint">{{x "このブラウザでは共有画面を直接開けないため、ファイルはダウンロードされます。" "This browser cannot open the share sheet directly, so files will be downloaded instead."}}</div><div id="files" class="files"><div class="empty">{{t "phone.checking"}}</div></div></section>
...
function detectShareSupport(){try{const sample=new File(['x'],'x.txt',{type:'text/plain'});const data={files:[sample]};shareFilesSupported=!!navigator.share&&(!navigator.canShare||navigator.canShare(data))}catch(_){shareFilesSupported=false}receiveHint.style.display=shareFilesSupported?'none':'block'}
