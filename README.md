...
- 起動からの絶対時間上限は設けない
- 定期的な画面更新は実操作として扱わない
- 残り時間は常時表示せず、無操作終了の10分前からだけ注意を表示
...
標準設定は `idleTimeoutMinutes: 180` です。

最後の実操作から3時間操作がない場合に転送セッションを自動終了します。起動からの絶対時間上限はありません。
...
アップロード・ダウンロード中はデータが継続して流れている限り一定間隔で実操作として更新されます。

画面には残り時間を常時表示せず、無操作終了まで10分以下になった場合だけ警告します。
...
完了済みの受信ファイルは残ります。PC→スマホ用の一時共有ファイルは次回起動時にクリアされます。

Wi-Fiパスワード、転送URL、セッショントークンは次回起動時にすべて新しく生成されます。
