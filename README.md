...
# VMware / 仮想マシンでの安全なテスト

Wi-Fi Directを仮想マシンへ渡せない環境では、リポジトリ直下の

```text
LocalPhoneTransfer-Test.cmd
```

を実行します。

このテストモードは本番モードとはネットワーク境界を分離しています。

- Wi-Fi Directを起動しない
- Windowsファイアウォールの設定を変更しない
- HTTPサーバーを **`127.0.0.1` のみにバインド**
- VMwareがNAT / ブリッジ接続でもLANには公開しない
- 管理画面は従来どおりループバック接続のみ許可
- 管理URL・スマホ画面URLは起動ごとのランダムトークンを使用
- PC側画面とスマホ側画面をVM内ブラウザで同時に開く

そのためテストモードでは、実際のスマートフォンやホストPCから接続することはできません。VM内でUI、アップロード、ダウンロード、ファイル一覧、終了処理などを確認するためのモードです。

実機でのWi-Fi Direct接続確認は、本番Windows PCで通常の `Local Phone Transfer` から行ってください。
...
- HTTPレスポンスにCSP・Permissions-Policy等のセキュリティヘッダーを付与
- VMテストモードは `127.0.0.1` のみに待ち受け、LANへ公開しない
